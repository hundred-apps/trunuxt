<template>
  <section class="mb-8 lg:mb-12" aria-labelledby="products-heading">
    <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h2 id="products-heading" class="text-xl lg:text-2xl font-bold text-gray-800">
          {{ $t('page.category.products', { count: totalProducts }) }}
        </h2>
        <p class="text-sm text-gray-500 mt-1">
          {{ $t('page.category.showing', { from: showingFrom, to: showingTo, total: totalProducts }) }}
        </p>
      </div>
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <el-select
          ...
          placeholder="Urutkan"
        >
          <el-option :label="$t('page.category.sort.default')" value="default" />
          <el-option :label="$t('page.category.sort.priceLow')" value="price_asc" />
          <el-option :label="$t('page.category.sort.priceHigh')" value="price_desc" />
    ...
    <div v-else-if="products.length === 0" class="text-center py-16 bg-white rounded-xl shadow-sm">
      <Icon name="material-symbols:inventory-2" class="text-6xl text-gray-300 mb-4" />
    ...
    <div v-else>
      <!-- Grid View -->
      <div v-if="viewMode === 'grid'" class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
        <ProductCard
...
const showingTo = computed(() => Math.min(props.currentPage * props.perPage, props.totalProducts));
