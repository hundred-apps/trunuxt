<template>
  <div class="admin-brands">
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ $t('admin.brands.title') }}</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-6">
        <div v-for="i in 8" :key="i" class="animate-pulse bg-gray-50 rounded-lg p-4 border border-gray-200 text-center">
          <div class="aspect-square bg-gray-200 rounded mx-auto mb-3 max-w-xs" />
          <div class="h-4 bg-gray-200 rounded w-3/4 mx-auto" />
        </div>
      </div>

      <div v-else-if="brands.length === 0" class="text-center py-16">
        <Icon name="material-symbols:label" class="text-4xl text-gray-300 mx-auto mb-3" />
        <h3 class="text-lg font-medium text-gray-900 mb-1">{{ $t('admin.brands.empty') }}</h3>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-6">
        <div
        ...
        >
          <div class="aspect-square mb-3 overflow-hidden rounded">
            ...
            />
            <div v-else class="aspect-square bg-gray-200 rounded flex items-center justify-center">
              <Icon name="material-symbols:label" class="h-12 w-12 text-gray-400" />
            </div>
          </div>
          <h3 class="font-bold text-gray-800 line-clamp-1 mb-1">{{ brand.name }}</h3>
          <p class="text-sm text-gray-500 mb-3">{{ brand.categoryCount }} kategori</p>
...
const fetchBrands = async () => {
  ...
  try {
    ...
    brands.value = [
      ...
      { id: 6, name: 'CATERPILLAR', logo: 'caterpillar.png', categoryCount: 12 },
