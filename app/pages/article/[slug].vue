        ...
        <div class="container">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <div class="lg:col-span-4">
              <div class="sticky top-[var(--header-height,80px)] flex flex-col gap-4" style="--header-height: 80px;">
                <!-- Trending Section -->
...
useSeoMeta({
  ...
  description: computed(() => article.value?.excerpt || article.value?.content?.replace(/<[^>]*>/g, "").substring(0, 160) || ""),
  ogDescription: computed(() => article.value?.excerpt || article.value?.content?.replace(/<[^>]*>/g, "").substring(0, 160) || ""),
...
useSchemaOrg([
  defineBreadcrumb({
    itemListElement: [
      ...
      { position: 3, name: computed(() => article.value?.title || "Artikel"), item: computed(() => `https://www.trumecs.com/article/${article.value?.url}`) },
    ],
  }),
]);
